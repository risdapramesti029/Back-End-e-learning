...
export class AuthController {
  ...
  async getAllUser(@Res() res) {
    try {
      const [user]: any = await this.authService.getAllUser();
      const data = user.map((pembayaran) => {
        pembayaran.file_url = `http://localhost:3000/static/${pembayaran.file}`;
        return pembayaran;
      });
  ...
  async getUserById(@Res() res, @Param('id') id) {
    try {
      ...
      const pembayaran: any = await this.authService.getPembayaranById(user);
      const file_pembayaran = pembayaran
        ? `http://localhost:3000/static/${pembayaran.file}`
        : null;
      ...
      return this.responseHelper.responseSuccessData(
        ...
        { user, file_pembayaran, mapel, materi, soal },
      );
    } catch (error) {
      console.log(error);
      return this.responseHelper.responseServerError(res);
